...
在前面第 9 章我们学了 `__init__`（初始化），这里需要了解它的"搭档" `__new__`（创建）：

```python
# 对象创建的两个步骤：
# 1. __new__(cls)  → 创建对象（分配内存），返回对象
# 2. __init__(self) → 初始化对象（设置属性）
# 
# 平时我们只写 __init__，因为 __new__ 有默认实现。
# 但当你需要控制"是否创建新对象"时，就要重写 __new__。
#
# 对比：
# __init__：每次都执行，不能阻止创建
# __new__：在创建之前执行，可以返回已有对象（从而阻止创建新对象）

class MyClass:
    def __new__(cls):
        print("1. __new__：创建对象")
        obj = super().__new__(cls)    # 调用父类创建对象
        return obj
...
> 简单记：`__new__` 决定**"创建哪个对象"**，`__init__` 决定**"怎么初始化对象"**。单例模式就是利用 `__new__` 让它返回同一个对象。
